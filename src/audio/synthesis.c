...
u64 *synthesis_process_note(struct Note *note, struct NoteSubEu *noteSubEu,
                            struct NoteSynthesisState *synthesisState, u16 *aiBuf, s32 bufLen,
                            u64 *cmd);
...
void prepare_reverb_ring_buffer(s32 chunkLen, u32 updateIndex, s32 reverbIndex) {
    ...
    if (reverb->downsampleRate != 1) {
        if (reverb->framesLeftToIgnore == 0) {
            ...
            for (dstPos = 0; dstPos < item->lengths[1] / 2;
                 srcPos += reverb->downsampleRate, dstPos++) {
...
u64 *synthesis_execute(u64 *cmdBuf, s32 *writtenCmds, u16 *aiBuf, s32 bufLen) {
    ...
    for (i = gAudioBufferParameters.updatesPerFrame; i > 0; i--) {
        ...
        cmd = synthesis_do_one_audio_update((u16 *) aiBufPtr, chunkLen, cmd,
...
u64 *synthesis_resample_and_mix_reverb(u64 *cmd, s32 bufLen, s16 reverbIndex, s16 updateIndex) {
    ...
    if (gSynthesisReverbs[reverbIndex].downsampleRate == 1) {
        cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH, item->startPos,
                                                item->lengths[0], reverbIndex);
        if (item->lengths[1] != 0) {
            cmd = synthesis_load_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH + item->lengths[0], 0,
                                                    item->lengths[1], reverbIndex);
        }
        ...
        cmd = synthesis_load_reverb_ring_buffer(cmd, 0x20, (item->startPos - temp_t9 / 2),
                                                DEFAULT_LEN_1CH, reverbIndex);
        if (item->lengths[1] != 0) {
            cmd = synthesis_load_reverb_ring_buffer(cmd, 0x20 + sp58, 0, DEFAULT_LEN_1CH - sp58,
                                                    reverbIndex);
        }
...
u64 *synthesis_save_reverb_samples(u64 *cmdBuf, s16 reverbIndex, s16 updateIndex) {
    ...
    if (reverb->useReverb != 0) {
        ...
        if (reverb->downsampleRate == 1) {
            ...
            cmd = cmdBuf = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH, item->startPos,
                                                             item->lengths[0], reverbIndex);
            if (item->lengths[1] != 0) {
                // Ring buffer wrapped
                cmd = synthesis_save_reverb_ring_buffer(cmd, DMEM_ADDR_WET_LEFT_CH + item->lengths[0],
                                                        0, item->lengths[1], reverbIndex);
                cmdBuf = cmd;
            }
        } else {
            ...
            aSaveBuffer(cmdBuf++, VIRTUAL_TO_PHYSICAL2(
...
u64 *synthesis_process_note(struct Note *note, struct NoteSubEu *noteSubEu,
                            struct NoteSynthesisState *synthesisState, UNUSED u16 *aiBuf, s32 bufLen,
                            u64 *cmd) {
    UNUSED s32 pad0[3];
#else
u64 *synthesis_process_notes(u16 *aiBuf, s32 bufLen, u64 *cmd) {
